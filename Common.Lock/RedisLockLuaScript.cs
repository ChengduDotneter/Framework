namespace Common.Lock
{
    ...
    public static class RedisLockLuaScript
    {
        ...
        /// 多读多写锁申请脚本
        /// </summary>
        internal const string ACQUIRE_NREAD_NWRITE_LOCK = @"-- 多读多写读写锁申请
...
return is_locked";
        ...
        /// 一写多读读写锁申请脚本
        /// </summary>
        internal const string ACQUIRE_NREAD_ONEWRITE_LOCK = @"-- 一写多读读写锁申请
local transcation_id = KEYS[1] -- 事务ID
local exp_time = KEYS[2] -- 超时时间 单位 毫秒
...
return is_locked";
        ...
        /// 读写锁释放脚本
        /// </summary>
        internal const string Release_DB_LOCK = @"-- 锁释放
local transcation_id = KEYS[1] -- 事务ID
...
if( lock_resources_length > 0 ) then
	for i = 3,3 + lock_resources_length - 1 do -- 对释放的行锁资源进行循环
        ...
        /// 读写锁维持脚本
        /// </summary>
        internal const string SAVE_DB_LOCK = @"-- 锁维持
