...
{
    public enum QueueDataTypeEnum2
    {
        /// <summary>
        /// 申请
        /// </summary>
    ...
    public struct EnQueueData2
    {
        ...
        public QueueDataTypeEnum2 QueueDataType { get; set; }
    }

    public struct DeQueueData2
    {
        public long Identity { get; set; }
    ...
    /// 死锁监测Grain类，单例
    /// </summary>
    public class ResourceDetection : IDeadlockDetection, IHostedService
    {
        /// <summary>
        /// 默认事务资源数组长度
        /// </summary>
        ...
        private static ConcurrentQueue<EnQueueData2> m_enQueueDatas;

        public delegate void DeQueueHandle(DeQueueData2 deQueueData);
        ...
        public ResourceDetection()
        {
            ...
            m_enQueueDatas = new ConcurrentQueue<EnQueueData2>();
        }
        ...
        private Task RunDeadLockDetection()
        {
            return Task.Factory.StartNew(() =>
            {
                while (true)
                {
                    if (m_enQueueDatas.TryDequeue(out EnQueueData2 enQueueData))
                        switch (enQueueData.QueueDataType)
                        {
                            case QueueDataTypeEnum2.Apply:
                                AddTranResource(enQueueData.Identity, enQueueData.ResourceName, enQueueData.Weight);
                                break;

                            case QueueDataTypeEnum2.Release:
                                RemoveTranResource(enQueueData.Identity);
                                break;
        ...
        public static Task EnQueued(EnQueueData2 enQueueData)
        {
        ...
        private void DeQueueChange(DeQueueData2 deQueueData)
        {
        ...
        /// <returns></returns>
        public Task AddTranResource(long identity, string resourceName, int weight)
        {
            int identityIndex;
        ...
        /// <param name="resourceName">事务申请的资源名，现包括数据表名</param>
        /// <returns></returns>
        public Task RemoveTranResource(long identity)
        {
            if (m_identityIndexs.ContainsKey(identity))
            {
                ...
                DeQueueChange(new DeQueueData2() { Identity = identity, QueueDataType = QueueDataTypeEnum2.Release });
            }
        ...
        private void CheckLock(int lastIdentityIndex, int lastResourceNameIndex)
        {
            ...
            GetNextDoIdentity(lastResourceNameIndex);
        }

        private void GetNextDoIdentity(int lastResourceNameIndex)
        {
            int minIdentityIndex = 0;

            foreach (var item in m_identityKeyIndexs)
            {
                if (m_matrix[minIdentityIndex, lastResourceNameIndex] == 0)
                    minIdentityIndex = item.Key;
                else if (m_matrix[item.Key, lastResourceNameIndex] > 0 && m_matrix[item.Key, lastResourceNameIndex] <= m_matrix[minIdentityIndex, lastResourceNameIndex])
                    minIdentityIndex = item.Key;
            }

            //Console.WriteLine($" RESOURCE {m_resourceNameKeyIndexs[lastResourceNameIndex]} NEXT DO Identity {m_identityKeyIndexs[minIdentityIndex]}");

            DeQueueChange(new DeQueueData2()
            {
                Identity = m_identityKeyIndexs[minIdentityIndex],
                QueueDataType = QueueDataTypeEnum2.Apply,
                ResourceName = m_resourceNameKeyIndexs[lastResourceNameIndex]
            });
        }
        ...
        private void ConflictResolution(int identityIndexA, int identityIndexB, int resourceIndexA, int resourceIndexB)
        {
            ...
            int continueResourceIndex;

            if (m_weights[identityIndexA] >= m_weights[identityIndexB])
            {
                destoryIdentity = m_identityKeyIndexs[identityIndexB];
                continueResourceIndex = resourceIndexA;
            }
            else
            {
                destoryIdentity = m_identityKeyIndexs[identityIndexA];
                continueResourceIndex = resourceIndexB;
            }

            GetNextDoIdentity(continueResourceIndex);
            DeQueueChange(new DeQueueData2() { Identity = destoryIdentity, QueueDataType = QueueDataTypeEnum2.Release });
        }
